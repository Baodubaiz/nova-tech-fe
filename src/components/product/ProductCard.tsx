...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="group relative flex flex-col bg-white border border-slate-200 rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
      <Link href={`/product/${product.id}`} className="no-underline flex flex-col h-full text-slate-800">
        {/* Product Image */}
        <div className="relative aspect-square w-full bg-white overflow-hidden flex items-center justify-center p-4">
          ...
          <img 
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=300&q=80';
            }}
          />
        </div>
      ...
      {/* Action Section (Outside Link to avoid nested button click redirection) */}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="p-2 border border-blue-600 hover:bg-blue-50 text-blue-600 disabled:border-slate-200 disabled:text-slate-400 disabled:hover:bg-transparent rounded flex items-center justify-center cursor-pointer transition-colors bg-white"
            >
